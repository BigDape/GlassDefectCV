...
void LightControl::initLineEditInputType()
{
    ...
    ui->EncodePressureWheelCountALE->setValidator(lineInt);
    ui->EncodePressureWheelCountBLE->setValidator(lineInt);
    ui->PressureWheel4FrequencyMutliperLE->setValidator(lineInt);
    ui->PressureEncodecoefficientLE->setValidator(lineDouble);
    //控制器参数
...
void LightControl::slotSetBTSystemName()
{
    // 设置全局变量
    ...
    // 写入文件
    m_root["SystemParams"]["SystemName"] = ui->SystemNameLE->text().toStdString().data();
    LightControl::writeCurrentRooterToJson();
}
...
void LightControl::slotSetBTCamera0Name()
{
    // 设置全局变量
    PARAM.setCamera0Name(ui->Camera0NameLE->text());
    // 弹出对话框
...
void LightControl::slotSetBTCamera1Name()
{
    // 设置全局变量
    PARAM.setCamera1Name(ui->Camera1NameLE->text().toStdString().data());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["SystemParams"]["Camare1Name"] = ui->Camera1NameLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera2Name()
{
    // 设置全局变量
    PARAM.setCamera2Name(ui->Camera2NameLE->text().toStdString().data());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["SystemParams"]["Camare2Name"] = ui->Camera2NameLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera3Name()
{
    // 设置全局变量
    PARAM.setCamera3Name(ui->Camera3NameLE->text().toStdString().data());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["SystemParams"]["Camare3Name"] = ui->Camera3NameLE->text().toStdString().data();
...
void LightControl::slotSetBTServerIP()
{
    // 设置全局变量
    PARAM.setServerIP(ui->ServerIPLE->text());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["SystemParams"]["ServerIP"] = ui->ServerIPLE->text().toStdString().data();
...
{
    // 设置全局变量
    PARAM.setPort(ui->PortLE->text().toInt());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"设置对话框","设置成功");
    // 写入文件
    m_root["SystemParams"]["ServerPort"] = ui->PortLE->text().toStdString().data();
...
void LightControl::slotSetBTSaveOriginImage()
{
    // 设置全局变量
    PARAM.setSaveOriginImage(ui->SaveOriginImageCB->currentIndex());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CustomParams"]["SaveOriginImage"] = QString::number(ui->SaveOriginImageCB->currentIndex()).toStdString().data();
...
void LightControl::slotSetBTSaveEntireImage()
{
    // 设置全局变量
    PARAM.setSaveEntireImage(ui->SaveEntireImageCB->currentIndex());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CustomParams"]["SaveEntireImage"] = QString::number(ui->SaveEntireImageCB->currentIndex()).toStdString().data();
...
void LightControl::slotSetBTSaveFlawImage()
{
    // 设置全局变量
    PARAM.setSaveFlawImage(ui->SaveFlawImageCB->currentIndex());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CustomParams"]["SaveFlawImage"] = QString::number(ui->SaveFlawImageCB->currentIndex()).toStdString().data();
...
void LightControl::slotSetBTSaveCropImage()
{
    // 设置全局变量
    PARAM.setSaveCropImage(ui->SaveCropImageCB->currentIndex());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CustomParams"]["SaveCropImage"] = QString::number(ui->SaveCropImageCB->currentIndex()).toStdString().data();
...
void LightControl::slotSetBTXYLengthEnable()
{
    // 设置全局变量
    PARAM.setXYLengthEnable(ui->XYLengthEnableCB->currentIndex());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CustomParams"]["XYLengthEnable"] = QString::number(ui->XYLengthEnableCB->currentIndex()).toStdString().data();
...
void LightControl::slotSetBTFlawDefectEnable()
{
    // 设置全局变量
    PARAM.setFlawDefectEnable(ui->FlawDefectEnableCB->currentIndex());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CustomParams"]["FlawDefectEnable"] = QString::number(ui->FlawDefectEnableCB->currentIndex()).toStdString().data();
...
void LightControl::slotSetBTCamera12PixelDeviation()
{
    // 设置全局变量
    PARAM.setCamera12PixelDeviation(ui->Camera12PixelDeviationLE->text().toInt());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DefectDetectionParams"]["Camera12PixelDeviation"] = ui->Camera12PixelDeviationLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera23PixelDeviation()
{
    // 设置全局变量
    PARAM.setCamera23PixelDeviation(ui->Camera23PixelDeviationLE->text().toInt());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DefectDetectionParams"]["Camera23PixelDeviation"] = ui->Camera23PixelDeviationLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera24PixelDeviation()
{
    //设置全局变量
    PARAM.setCamera24PixelDeviation(ui->Camera24PixelDeviationLE->text().toInt());
    // 弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DefectDetectionParams"]["Camera24PixelDeviation"] = ui->Camera24PixelDeviationLE->text().toStdString().data();
...
void LightControl::slotSetBTSilkToRollerDist()
{
    //设置全局变量
    PARAM.setSilkToRollerDist(ui->SilkToRollerDistLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DefectDetectionParams"]["SilkToRollerDist"] = ui->SilkToRollerDistLE->text().toStdString().data();
...
void LightControl::slotSetBTScratchAreaThreshold()
{
    //设置全局变量
    PARAM.setScratchAreaThreshold(ui->ScratchAreaThresholdLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DefectDetectionParams"]["ScratchAreaThreshold"] = ui->ScratchAreaThresholdLE->text().toStdString().data();
...
void LightControl::slotSetBTBubbleStoneAreaThreshold()
{
    //设置全局变量
    PARAM.setBubbleStoneAreaThreshold(ui->BubbleStoneAreaThresholdLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DefectDetectionParams"]["BubbleStoneAreaThreshold"] = ui->BubbleStoneAreaThresholdLE->text().toStdString().data();
...
void LightControl::slotSetBTDirtyAreaThreshold()
{
    //设置全局变量
    PARAM.setDirtyAreaThreshold(ui->DirtyAreaThresholdLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DefectDetectionParams"]["DirtyAreaThreshold"] = ui->DirtyAreaThresholdLE->text().toStdString().data();
...
void LightControl::slotSetBTYAccuracyB()
{
    //设置全局变量
    PARAM.setYAccuracyB(ui->YAccuracyBLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["YAccuracyB"] = ui->YAccuracyBLE->text().toStdString().data();
...
void LightControl::slotSetBTYAccuracyK()
{
    // 设置全局变量
    PARAM.setYAccuracyK(ui->YAccuracyKLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["YAccuracyK"] = ui->YAccuracyKLE->text().toStdString().data();
...
void LightControl::slotSetBTWidth()
{
    // 设置全局变量
    PARAM.setWidth(ui->WidthLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Width"] = ui->WidthLE->text().toStdString().data();
...
void LightControl::slotSetBTWidthDeviation()
{
    // 设置全局变量
    PARAM.setWidthDeviation(ui->WidthDeviationLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["WidthDeviation"] = ui->WidthDeviationLE->text().toStdString().data();
...
void LightControl::slotSetBTDigonal()
{
    // 设置全局变量
    PARAM.setDigonal(ui->Digonal1LE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Digonal1"] = ui->Digonal1LE->text().toStdString().data();
...
void LightControl::slotSetBTDigonal1Deviation()
{
    // 设置全局变量
    PARAM.setDigonal1Deviation(ui->Digonal1DeviationLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Digonal1Deviation"] = ui->Digonal1DeviationLE->text().toStdString().data();
...
void LightControl::slotSetBTDigonal2()
{
    // 设置全局变量
    PARAM.setDigonal2(ui->Digonal2LE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Digonal2"] = ui->Digonal2LE->text().toStdString().data();
    LightControl::writeCurrentRooterToJson();
}

void LightControl::slotSetBTDigonal2Deviation()
{
    // 设置全局变量
    PARAM.setDigonal2Deviation(ui->Digonal2DeviationLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Digonal2Deviation"] = ui->Digonal2DeviationLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera1Pixel0Accuracy()
{
    // 设置全局变量
    PARAM.setCamera1Pixel0Accuracy(ui->Camera1Pixel0AccuracyLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Camera1Pixel0Accuracy"] = ui->Camera1Pixel0AccuracyLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera1PixelKValue()
{
    // 设置全局变量
    PARAM.setCamera1PixelKValue(ui->Camera1PixelKValueLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Camera1PixelKValue"] = ui->Camera1PixelKValueLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera21AccuracyRatio()
{
    // 设置全局变量
    PARAM.setCamera21AccuracyRatio(ui->Camera21AccuracyRatioLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Camera21AccuracyRatio"] = ui->Camera21AccuracyRatioLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera2PixelKValue()
{
    // 设置全局变量
    PARAM.setCamera2PixelKValue(ui->Camera2PixelKValueLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Camera2PixelKValue"] = ui->Camera2PixelKValueLE->text().toStdString().data();
...
void LightControl::slotSetBTLength()
{
    // 设置全局变量
    PARAM.setLength(ui->LengthLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["Length"] = ui->LengthLE->text().toStdString().data();
...
void LightControl::slotSetBTLengthDeviation()
{
    // 设置全局变量
    PARAM.setLengthDeviation(ui->LengthDeviationLE->text().toDouble());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["DimensionalMeasurementParams"]["LengthDeviation"] = ui->LengthDeviationLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera0Frame()
{
    // 设置全局变量
    PARAM.setCamera0Frame(ui->Camera0FrameLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera0Frame"] = ui->Camera0FrameLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera0PhotoRow()
{
    // 设置全局变量
    PARAM.setCamera0PhotoRow(ui->Camera0PhotoRowLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera0PhotoRow"] = ui->Camera0PhotoRowLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera0ExposureTime()
{
    // 设置全局变量
    PARAM.setCamera0ExposureTime(ui->Camera0ExposureTimeLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera0ExposureTime"] = ui->Camera0ExposureTimeLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera0Gain()
{
    // 设置全局变量
    PARAM.setCamera0Gain(ui->Camera0GainLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera0Gain"] = ui->Camera0GainLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera1Frame()
{
    // 设置全局变量
    PARAM.setCamera1Frame(ui->Camera1FrameLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera1Frame"] = ui->Camera1FrameLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera1PhotoRow()
{
    // 设置全局变量
     PARAM.setCamera1PhotoRow(ui->Camera1PhotoRowLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera1PhotoRow"] = ui->Camera1PhotoRowLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera1ExposureTime()
{
    // 设置全局变量
    PARAM.setCamera1ExposureTime(ui->Camera1ExposureTimeLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera1ExposureTime"] = ui->Camera1ExposureTimeLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera1Gain()
{
    // 设置全局变量
    PARAM.setCamera1Gain(ui->Camera1GainLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera1Gain"] = ui->Camera1GainLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera2Frame()
{
    // 设置全局变量
    PARAM.setCamera2Frame(ui->Camera2FrameLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera2Frame"] = ui->Camera2FrameLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera2PhotoRow()
{
    // 设置全局变量
     PARAM.setCamera2PhotoRow(ui->Camera2PhotoRowLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera2PhotoRow"] = ui->Camera2PhotoRowLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera2ExposureTime()
{
    // 设置全局变量
    PARAM.setCamera2ExposureTime(ui->Camera2ExposureTimeLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera2ExposureTime"] = ui->Camera2ExposureTimeLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera2Gain()
{
    // 设置全局变量
    PARAM.setCamera2Gain(ui->Camera2GainLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera2Gain"] = ui->Camera2GainLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera3Frame()
{
    // 设置全局变量
    PARAM.setCamera3Frame(ui->Camera3FrameLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera3Frame"] = ui->Camera3FrameLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera3PhotoRow()
{
    // 设置全局变量
    PARAM.setCamera3PhotoRow(ui->Camera3PhotoRowLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera3PhotoRow"] = ui->Camera3PhotoRowLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera3ExposureTime()
{
    // 设置全局变量
    PARAM.setCamera3ExposureTime(ui->Camera3ExposureTimeLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera3ExposureTime"] = ui->Camera3ExposureTimeLE->text().toStdString().data();
...
void LightControl::slotSetBTCamera3Gain()
{
    // 设置全局变量
    PARAM.setCamera3Gain(ui->Camera3GainLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["CamareParams"]["Camera3Gain"] = ui->Camera3GainLE->text().toStdString().data();
...
/////////////////////////////////////////////////////////////
void LightControl::slotSetBTEncodeUnitToDist()
{
    // 设置全局变量
    PARAM.setEncodeUnitToDist(ui->EncodeUnitToDistLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp0 = PARAM.getEncodeUnitToDist();
    m_pSig_comm.SetRegs(ADDR0, ADDR0, &sigctrl->temp0);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTEncodePulseFilterUs()
{
    // 设置全局变量
    PARAM.setEncodePulseFilterUs(ui->EncodePulseFilterUsLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp1 = PARAM.getEncodePulseFilterUs();
    m_pSig_comm.SetRegs(ADDR1, ADDR1, &sigctrl->temp1);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTEncodeCountEndFlag()
{
    // 设置全局变量
    PARAM.setEncodeCountEndFlag(ui->EncodeCountEndFlagLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["EncodeCountEndFlag"] = ui->EncodeCountEndFlagLE->text().toStdString().data();
...
void LightControl::slotSetBTEncodeCount()
{
    // 设置全局变量
    PARAM.setEncodeCount(ui->EncodeCountLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["EncodeCount"] = ui->EncodeCountLE->text().toStdString().data();
...
void LightControl::slotSetBTEncodeTriggerRowCount()
{
    // 设置全局变量
    PARAM.setEncodeTriggerRowCount(ui->EncodeTriggerRowCountLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["EncodeTriggerRowCount"] = ui->EncodeTriggerRowCountLE->text().toStdString().data();
...
void LightControl::slotSetBTEncode4FrequencyMultiplierCount()
{
    // 设置全局变量
    PARAM.setEncode4FrequencyMultiplierCount(ui->Encode4FrequencyMultiplierCountLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["Encode4FrequencyMultiplierCount"] = ui->Encode4FrequencyMultiplierCountLE->text().toStdString().data();
...
void LightControl::slotSetBTEncodeUnitTimePulseCount()
{
    // 设置全局变量
    PARAM.setEncodeUnitTimePulseCount(ui->EncodeUnitTimePulseCountLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["EncodeUnitTimePulseCount"] = ui->EncodeUnitTimePulseCountLE->text().toStdString().data();
...
void LightControl::slotSetBTEncodeCountTimeHalfSecond()
{
    // 设置全局变量
    PARAM.setEncodeCountTimeHalfSecond(ui->EncodeCountTimeHalfSecondLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp7 = PARAM.getEncodeCountTimeHalfSecond();
    m_pSig_comm.SetRegs(ADDR7, ADDR7, &sigctrl->temp7);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTEncodeChannel()
{
    // 设置全局变量
    PARAM.setEncodeChannel(ui->EncodeChannelLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp8 = PARAM.getEncodeChannel();
    m_pSig_comm.SetRegs(ADDR8, ADDR8, &sigctrl->temp8);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTEncodeRevolution()
{
    // 设置全局变量
    PARAM.setEncodeRevolution(ui->EncodeRevolutionLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp9 = PARAM.getEncodeRevolution();
    m_pSig_comm.SetRegs(ADDR9, ADDR9, &sigctrl->temp9);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTRollerPerimeterUm()
{
    // 设置全局变量
    PARAM.setRollerPerimeterUm(ui->RollerPerimeterUmLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp10 = PARAM.getRollerPerimeterUm();
    m_pSig_comm.SetRegs(ADDR10, ADDR10, &sigctrl->temp10);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTExpectSinglePixelAccuracy()
{
    // 设置全局变量
    PARAM.setExpectSinglePixelAccuracy(ui->ExpectSinglePixelAccuracyLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp11 = PARAM.getExpectSinglePixelAccuracy();
    m_pSig_comm.SetRegs(ADDR11, ADDR11, &sigctrl->temp11);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTRollerMaxSpeedToRowCount()
{
    // 设置全局变量
    PARAM.setRollerMaxSpeedToRowCount(ui->RollerMaxSpeedToRowCountLE->text().toInt());
    // 发送设置值给信号平台
    sigctrl->temp12 = PARAM.getRollerMaxSpeedToRowCount();
    m_pSig_comm.SetRegs(ADDR12, ADDR12, &sigctrl->temp12);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTEveryRowToEncodeCount()
{
    // 设置全局变量
    PARAM.setEveryRowToEncodeCount(ui->EveryRowToEncodeCountLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["EveryRowToEncodeCount"] = ui->EveryRowToEncodeCountLE->text().toStdString().data();
...
void LightControl::slotSetBTEncodePressureWheelCountA()
{
    // 设置全局变量
    PARAM.setEncodePressureWheelCountA(ui->EncodePressureWheelCountALE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["EncodePressureWheelCountA"] = ui->EncodePressureWheelCountALE->text().toStdString().data();
...
void LightControl::slotSetBTEncodePressureWheelCountB()
{
    // 设置全局变量
    PARAM.setEncodePressureWheelCountB(ui->EncodePressureWheelCountBLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["EncodePressureWheelCountB"] = ui->EncodePressureWheelCountBLE->text().toStdString().data();
...
void LightControl::slotSetBTPressureWheel4FrequencyMutliper()
{
    // 设置全局变量
    PARAM.setPressureWheel4FrequencyMutliper(ui->PressureWheel4FrequencyMutliperLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["PressureWheel4FrequencyMutliper"] = ui->PressureWheel4FrequencyMutliperLE->text().toStdString().data();
...
void LightControl::slotSetBTPressureEncodecoefficient()
{
    // 设置全局变量
    PARAM.setPressureEncodecoefficient(ui->PressureEncodecoefficientLE->text().toInt());
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
    m_root["EncoderParams"]["PressureEncodecoefficient"] = ui->PressureEncodecoefficientLE->text().toStdString().data();
...
///////////////////////////////////////////////////////////////////////////////////////
void LightControl::slotSetBTPixelAccuracyUm()
{
    // 设置全局变量
    PARAM.setPixelAccuracyUm(ui->PixelAccuracyUmLE->text().toInt());
    sigctrl->temp14 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR14, ADDR14, &sigctrl->temp14);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTPhotoelectricSignalPulseFilter()
{
    // 设置全局变量
    PARAM.setPhotoelectricSignalPulseFilter(ui->PhotoelectricSignalPulseFilterLE->text().toInt());
    sigctrl->temp15 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR15, ADDR15, &sigctrl->temp15);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTCamareTrigerPulseContinueTime10ns()
{
    // 设置全局变量
    PARAM.setCamareTrigerPulseContinueTime10ns(ui->CamareTrigerPulseContinueTime10nsLE->text().toInt());
    sigctrl->temp16 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR16, ADDR16, &sigctrl->temp16);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTPhotoelectricToCamareDist()
{
    // 设置全局变量
    PARAM.setPhotoelectricToCamareDist(ui->PhotoelectricToCamareDistLE->text().toInt());
    sigctrl->temp17 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR17, ADDR17, &sigctrl->temp17);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTCamareFrameTrigerDelayRowCount()
{
    // 设置全局变量
    PARAM.setCamareFrameTrigerDelayRowCount(ui->CamareFrameTrigerDelayRowCountLE->text().toInt());
    sigctrl->temp18 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR18, ADDR18, &sigctrl->temp18);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTFrameSignalContinueTime()
{
    // 设置全局变量
    PARAM.setFrameSignalContinueTime(ui->FrameSignalContinueTimeLE->text().toInt());
    sigctrl->temp19 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR19, ADDR19, &sigctrl->temp19);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTModuleEnableSignal()
{
    // 设置全局变量
    PARAM.setModuleEnableSignal(ui->ModuleEnableSignalLE->text().toInt());
    sigctrl->temp20 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR20, ADDR20, &sigctrl->temp20);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTCamarePhotoRowCount()
{
    // 设置全局变量
    PARAM.setCamarePhotoRowCount(ui->CamarePhotoRowCountLE->text().toInt());
    sigctrl->temp21 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR21, ADDR21, &sigctrl->temp21);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTInnerRowFrequency()
{
    // 设置全局变量
    PARAM.setInnerRowFrequency(ui->InnerRowFrequencyLE->text().toInt());
    sigctrl->temp22 = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR22, ADDR22, &sigctrl->temp22);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTPhotoMode()
{
    // 设置全局变量
    PARAM.setPhotoMode(ui->PhotoModeLE->text().toInt());
    sigctrl->tempAutoPhoto = PARAM.getPressureEncodecoefficient();
    m_pSig_comm.SetRegs(ADDR59, ADDR59, &sigctrl->tempAutoPhoto);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTPhotoEndDelayRowCount()
{
    // 设置全局变量
    PARAM.setPhotoEndDelayRowCount(ui->PhotoEndDelayRowCountLE->text().toInt());
    sigctrl->tempPhotoEndDelayLine = PARAM.getPressureEncodecoefficient();
...
/////////////////////////////////////////////////////////////////////////////
void LightControl::slotSetBTLightField1DelayTime()
{
    // 设置全局变量
    PARAM.setLightField1DelayTime(ui->LightField1DelayTimeLE->text().toInt());
    sigctrl->temp23 = PARAM.getLightField1DelayTime();
    m_pSig_comm.SetRegs(ADDR23, ADDR23, &sigctrl->temp23);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTLightField1GlowTime()
{
    // 设置全局变量
    PARAM.setLightField1GlowTime(ui->LightField1GlowTimeLE->text().toInt());
    sigctrl->temp24 = PARAM.getLightField1GlowTime();
    m_pSig_comm.SetRegs(ADDR24, ADDR24, &sigctrl->temp24);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTLightField2DelayTime()
{
    // 设置全局变量
    PARAM.setLightField2DelayTime(ui->LightField2DelayTimeLE->text().toInt());
    sigctrl->temp25 = PARAM.getLightField2DelayTime();
    m_pSig_comm.SetRegs(ADDR25, ADDR25, &sigctrl->temp25);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTLightField2GlowTime()
{
    // 设置全局变量
    PARAM.setLightField2GlowTime(ui->LightField2GlowTimeLE->text().toInt());
    sigctrl->temp26 = PARAM.getLightField2GlowTime();
    m_pSig_comm.SetRegs(ADDR26, ADDR26, &sigctrl->temp26);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTLightField3DelayTime()
{
    // 设置全局变量
    PARAM.setLightField3DelayTime(ui->LightField3DelayTimeLE->text().toInt());
    sigctrl->temp27 = PARAM.getLightField3DelayTime();
    m_pSig_comm.SetRegs(ADDR27, ADDR27, &sigctrl->temp27);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTLightField3GlowTime()
{
    // 设置全局变量
    PARAM.setLightField3GlowTime(ui->LightField3GlowTimeLE->text().toInt());
    sigctrl->temp28 = PARAM.getLightField3GlowTime();
    m_pSig_comm.SetRegs(ADDR28, ADDR28, &sigctrl->temp28);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTLightField4DelayTime()
{
    // 设置全局变量
    PARAM.setLightField4DelayTime(ui->LightField4DelayTimeLE->text().toInt());
    sigctrl->temp29 = PARAM.getLightField4DelayTime();
    m_pSig_comm.SetRegs(ADDR29, ADDR29, &sigctrl->temp29);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTLightField4GlowTime()
{
    // 设置全局变量
    PARAM.setLightField4GlowTime(ui->LightField4GlowTimeLE->text().toInt());
    sigctrl->temp30 = PARAM.getLightField4GlowTime();
    m_pSig_comm.SetRegs(ADDR30, ADDR30, &sigctrl->temp30);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTSelectedLightFieldNumber()
{
    // 设置全局变量
    PARAM.setSelectedLightFieldNumber(ui->SelectedLightFieldNumberLE->text().toInt());
    sigctrl->temp55 = PARAM.getSelectedLightFieldNumber();
    m_pSig_comm.SetRegs(ADDR55, ADDR55, &sigctrl->temp55);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTHorizontalDarkfieldSelectRegister()
{
    // 设置全局变量
    PARAM.setHorizontalDarkfieldSelectRegister(ui->HorizontalDarkfieldSelectRegisterLE->text().toInt());
    sigctrl->temp56 = PARAM.getHorizontalDarkfieldSelectRegister();
    m_pSig_comm.SetRegs(ADDR56, ADDR56, &sigctrl->temp56);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTCamareAndLightFieldControl()
{
    // 设置全局变量
    PARAM.setCamareAndLightFieldControl(ui->CamareAndLightFieldControlCB->currentIndex());
    if (ui->CamareAndLightFieldControlCB->currentIndex() == 0) { //3fff
        sigctrl->temp35 = 0x3fff;
    } else if(ui->CamareAndLightFieldControlCB->currentIndex() == 1) { //7fff
        sigctrl->temp35 = 0x7fff;
    }
    m_pSig_comm.SetRegs(ADDR35, ADDR35, &sigctrl->temp35);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
...
void LightControl::slotSetBTSignalSwitch()
{
    // 设置全局变量
    PARAM.setSignalSwitch(ui->SignalSwitchLE->text().toInt());
    sigctrl->temp36 = PARAM.getSignalSwitch();
    m_pSig_comm.SetRegs(ADDR36, ADDR36, &sigctrl->temp36);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
...
void LightControl::slotSetBTRowSignalSelected()
{
    // 设置全局变量
    PARAM.setRowSignalSelected(ui->RowSignalSelectedLE->text().toInt());
    sigctrl->temp37 = PARAM.getRowSignalSelected();
    m_pSig_comm.SetRegs(ADDR37, ADDR37, &sigctrl->temp37);
    //弹出对话框
    INFOMATION.informationMessageBox(this,"保存对话框","保存成功");
    // 写入文件
