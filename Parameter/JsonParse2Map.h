...
class JsonParse2Map {
 public:
  ...
  bool DeleteParamTicket(QString name);
  ...
  void GetValueFromRecipes(QStringList KeyWord,QList<QList<Key2Value>>&ParamAll);
  void SetValue2Recipes(QStringList KeyWord,QList<QList<Key2Value>>&ParamAll);


  //新增函数计算玻璃ID对应的缺陷个数
  int CountValuesUnderKey(const QString& key) const;
