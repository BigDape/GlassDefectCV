...
#include "jsoncpp_global.h"
#include <json.h>
#include <QString>
#include <QDateTime>
#include "hstool.h"

struct JSONCPP_EXPORT DefectData{
    int DefectID; //序号
    QString Time; //时间
...
struct JSONCPP_EXPORT SizeData{
    int HolesID;// 序号
    QString Time;// 时间
    QString Type;// 类型
...
//单个孔/门夹/丝印
struct JSONCPP_EXPORT HoleData {
    QString HolesID;
    QString Time;
    QString Type;
    QString HolesLeve;
...
//单块玻璃写入json文件中的数据
struct JSONCPP_EXPORT HoleResult {
    int GlassID;
...
//单个缺陷
struct JSONCPP_EXPORT DefeteData{
    QString Time;
    int DefectID;
    QString DefectType;
    QString DetectLeve;
...
//单块玻璃写入json文件中的缺陷数据
struct JSONCPP_EXPORT DefeteResult{
    int GlassID;
...
//工单：光源控制器参数
struct JSONCPP_EXPORT LightControllerParameters{
...
//工单：尺寸测量
struct JSONCPP_EXPORT DimensionalMeasurementParameters{
    double YB;  //Y方向B
    double YK;  //Y方向精度K
    double width;   //宽度
...
//工单：控制器系统参数
struct JSONCPP_EXPORT ControllerSystemParameters {
    int PixAccury;                          //像素精度
...
//工单：相机参数
struct JSONCPP_EXPORT CameraParameters{
    int FrameCount;         //帧次
...
//工单：编码器参数
struct JSONCPP_EXPORT CoderParameters{
...
//工单：缺陷检测参数
struct JSONCPP_EXPORT DefectDetectionCoderParameters{
...
//工单：缺陷面积阈值
struct JSONCPP_EXPORT DefectAreaThresholdParameters{
    int HuashanAreaThreahold;        //划痕面积阈值
    int QijieAreaThreahold;         //气节面积阈值
    int ZanwuAreaThreahold;         //脏污面积阈值
};
//工单：自定义参数
struct JSONCPP_EXPORT CustomParameters{
    int SaveOrigin;                 //保存原图
    int SaveImage;                  //保存整图
...
//工单
struct JSONCPP_EXPORT WorkOrder{
...
class JSONCPP_EXPORT Jsoncpp
{
private:
    Jsoncpp();
public:
    static Jsoncpp* GetInstance();
    /**
     ...
     * @param ImagePath3
     */
    void getFlawImageFromJson(QString jsonFileName,
                              int GlassID,
                              int defectid,
                              QString& ImagePath1,
                              QString& ImagePath2,
     ...
     * @param Defects
     */
    void getDefectSFromJson(QString jsonFullPath,
                            int glassid,
                            std::vector<DefectData>& Defects);

    void readHoleFromJson(QString jsonFullPath, int glassid, QString& ImageHolesLinePath);

    void readSizeSmallImage(QString jsonFileName, int GlassID, int row, QString& ImageHolesPath);

    void readSizeFromJson(QString jsonFilePath,int glassid,std::vector<SizeData>& result);

    void writeSizeToJson(HoleResult inputdata);

    void writeDefectToJson(DefeteResult defect,int GlassID);

    //communication模块保存json
    void writeComDataToJson(QString recipeFullpath, const SignalControl& data);

    //main创建空白json工单
    void writeEmptyRecipeToJson(QString path);
    void readRecipeFromJson(QString path, SignalControl& signalCtrl);
    void writeRecipeToJson(QString fullpath, SignalControl signalCtrl);
};
