...
#include <Windows.h>
#include <QDebug>
#include "Dushen/MyGraphicsitem.h"
#include "DushenBasicFunc.h"

#pragma execution_character_set("utf-8")

DushenCameraWidget::DushenCameraWidget(QWidget* parent, DushenCameraArgs args)
    : QWidget(parent)
    , ui(new Ui::DushenCameraWidget)
{
    ...
    CameraBase = std::make_shared<DushenBasicFunc>(this,args);

    //在下拉框中添加相机名称
    for (dvpUint32 i = 0; i < CameraBase->CameraCounts; i++) {
        ui->comboBox_Devices->addItem(tr(CameraBase->info[i].FriendlyName));
    }
...
//    ui->comboBox_MultiFieldSelect->setCurrentIndex(Index4combox_MultiFieldSelect);
    ...
    // 点击属性按钮，操作
    connect(ui->pushButton_Property,SIGNAL(clicked()),this, SLOT(on_pushButton_Property_clicked()));
    // 点击选择按钮选择光场
    connect(ui->pushButton_MultiFieldSelect,SIGNAL(clicked()),this,SLOT(on_pushButton_MultiFieldSelect_clicked()));
    if (CameraBase->IsScanned) {
...
void DushenCameraWidget::AutoOpen()
{
    ...
    if (!CameraBase->IsOpened) {
        ...
        if (CameraBase->slot_OpenFunc(RealCameraName)) {
            loadedPixmapItem = new MyGraphicsItem();
            connect(CameraBase.get(), SIGNAL(sig_DeliverImage(QImage)), this, SLOT(slot_ShowImage(QImage)));              //更新图片
            connect(CameraBase.get(), SIGNAL(sig_DeliverFrameRate(QString)), this, SLOT(slot_ShowFrameRate(QString)));    //更新帧率
            ...
            CameraBase->slot_LoadIni(RealCameraName); //加载ini配置文件
        }
    }
}
...
void DushenCameraWidget::on_pushButton_Open_clicked()
{
    ...
    if (!CameraBase->IsOpened) {
        if (CameraBase->slot_OpenFunc(ui->comboBox_Devices->currentText())) {
            loadedPixmapItem = new MyGraphicsItem();
            scene->addItem(loadedPixmapItem);
            ...
            ui->comboBox_MultiFieldSelect->setEnabled(true);
            ui->pushButton_MultiFieldSelect->setEnabled(true);
            CameraBase->slot_LoadIni(RealCameraName);
        }
    } else {
...
//    Global::XmlParam.setParameter(t_CameraFieldName, CameraBase->m_AcquireImage->FieldSelectedView[m_CameraNum]);
...
{
    if (!CameraBase->IsStarted) {
        CameraBase->setFieldSelectedView(ui->comboBox_MultiFieldSelect->currentText().toInt());
    }
}
...
void DushenCameraWidget::setThreadSafeQueue(std::shared_ptr<ThreadSafeQueue<FrameImage> > queue)
{
   if(CameraBase!=nullptr) {
       CameraBase->setThreadSafeQueue(queue);
   }
}
